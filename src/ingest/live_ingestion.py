...
class Neo4jWriter:
    ...
    def write_batch(self, events: List[Dict[str, Any]]) -> bool:
        ...
        ON CREATE SET c.first_seen = event.timestamp, c.count = 1
        ON MATCH SET c.last_seen = event.timestamp, c.count = coalesce(c.count, 0) + 1
        
        MERGE (u)-[a:PERFORMED]->(act:Action {type: event.action})
        ON CREATE SET a.first_seen = event.timestamp, a.count = 1
        ON MATCH SET a.last_seen = event.timestamp, a.count = coalesce(a.count, 0) + 1
        
        MERGE (u)-[acc:ACCESSED]->(dst)
        ON CREATE SET acc.first_seen = event.timestamp
        ON MATCH SET acc.last_seen = event.timestamp
        """
...
class AlertEngine:
    ...
    def __init__(self):
        ...
        logger.info(f"Alert Engine initialized (Threshold: {Config.ALERT_THRESHOLD} evts / {Config.ALERT_WINDOW_SEC}s)")
    ...
    def trigger_webhook(self, node_id: str, count: int):
        ...
        try:
            ...
            if response.status_code == 200:
                logger.info(f"🚨 ALERT DISPATCHED: Node {node_id} exceeded threshold ({count} events).")
            else:
...
class KafkaConsumerManager:
    ...
    def start(self, message_callback: Callable[[List[str]], None]):
        ...
        try:
            self.consumer.subscribe([Config.KAFKA_TOPIC], on_assign=self.on_assign, on_revoke=self.on_revoke)
            self.running = True
    ...
    def initiate_fallback(self, message_callback: Callable[[List[str]], None]):
        ...
        for raw_payload in fallback.consume_generator():
            ...
            batch.append(raw_payload)
            if len(batch) >= Config.BATCH_SIZE or (time.time() - last_flush) >= Config.BATCH_TIMEOUT_SEC:
                if batch:
...
class IngestionPipeline:
    ...
    def process_batch(self, raw_messages: List[str]) -> bool:
        ...
        if success:
            ...
            logger.info(f"Processed batch of {len(processed_events)} events in {duration:.3f}s. "
